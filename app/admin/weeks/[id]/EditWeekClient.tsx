...
export default function EditWeekClient({ initialData, weekId }: { initialData: any, weekId: string }) {
  ...
  const [week, setWeek] = useState({
    ...
    opens_at: formatDateForInput(initialData.opens_at),
    closes_at: formatDateForInput(initialData.closes_at),
    pdf_url: initialData.pdf_url || '',
    pdf_filename: initialData.pdf_filename || '',
    xp_reward_full: initialData.xp_reward_full || 100,
    is_published: initialData.is_published || false,
  })

  const initialExams: ExamDraft[] = initialData.exams?.length > 0
    ? initialData.exams.map((exam: any) => ({
      id: exam.id,
      title: exam.title || '',
      description: exam.description || '',
      xp_reward_full: exam.xp_reward_full || 100,
      xp_reward_partial: exam.xp_reward_partial || 50,
  ...
  const [exams, setExams] = useState<ExamDraft[]>(initialExams)
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.error || 'تعذر تحديث الأسبوع')
        return
      }
    ...
    <main dir="rtl" style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #0f0c29 0%, #1a1040 45%, #0d1b3e 100%)', color: '#e2e8f0', fontFamily: "'Cairo', 'Segoe UI', sans-serif", padding: '28px 20px' }}>
      <section style={{ maxWidth: 980, margin: '0 auto' }}>
        ...
        <header style={{ margin: '22px 0' }}>
          <p style={{ margin: '0 0 8px', color: '#38bdf8', fontWeight: 800, fontSize: 13 }}>تعديل المحتوى</p>
          <h1 style={{ margin: 0, fontSize: 30, fontWeight: 800 }}>تعديل: {week.title}</h1>
        </header>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'grid', gap: 18 }}>
          ...
          <button type="submit" disabled={loading} style={{ justifySelf: 'start', color: '#fff', background: 'linear-gradient(135deg, #10b981, #059669)', border: 0, padding: '13px 20px', borderRadius: 14, fontFamily: 'inherit', fontWeight: 800, cursor: loading ? 'not-allowed' : 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
